...
from vingobot import __version__
from vingobot.bus.events import OutboundMessage
from vingobot.command.router import CommandContext, CommandRouter
from vingobot.utils.helpers import build_status_content
from vingobot.utils.restart import set_restart_notice_to_env
...
async def cmd_restart(ctx: CommandContext) -> OutboundMessage:
    ...
    async def _do_restart():
        ...
        os.execv(sys.executable, [sys.executable, "-m", "vingobot"] + sys.argv[1:])
...
async def cmd_status(ctx: CommandContext) -> OutboundMessage:
    ...
    with suppress(Exception):
        from vingobot.utils.searchusage import fetch_search_usage
...
async def cmd_dmn(ctx: CommandContext) -> OutboundMessage:
    """Manually trigger DMN cognitive maintenance mode."""
    import time
    ...
    async def _run_dmn():
        t0 = time.monotonic()
        try:
            await loop._run_dmn_turn("系统自我演化：内省与维护")
            elapsed = time.monotonic() - t0
            content = f"DMN completed in {elapsed:.1f}s."
        except Exception as e:
            elapsed = time.monotonic() - t0
            content = f"DMN failed after {elapsed:.1f}s: {e}"
        await loop.bus.publish_outbound(OutboundMessage(
            channel=msg.channel, chat_id=msg.chat_id, content=content,
        ))

    asyncio.create_task(_run_dmn())
    return OutboundMessage(
        channel=msg.channel, chat_id=msg.chat_id, content="Entering DMN mode...",
    )
...
def build_help_text() -> str:
    ...
    lines = [
        ...
        "/dmn — Enter DMN cognitive maintenance mode",
