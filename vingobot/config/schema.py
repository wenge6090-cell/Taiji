...
from vingobot.cron.types import CronSchedule
...
class SixiangAgentConfig(Base):
    """Per-agent model override for a sixiang agent.

    Each agent (mingjue, weaver, yang, yin, anqu) can have independent
    model parameters.  Unset fields fall through to ``agents.defaults``.
    """
...
class SixiangConfig(Base):
    """Sixiang (六爻) goal-driven loop configuration."""

    enabled: bool = False  # Master switch for the sixiang system
    max_concurrent_workers: int = Field(default=3, ge=1, le=20)
    max_task_rounds: int = Field(default=30, ge=1, le=200)
    max_goal_tasks: int = Field(default=100, ge=1, le=1000)
    max_rework_rounds: int = Field(default=3, ge=0, le=10)
    pending_poll_interval_ms: int = Field(default=500, ge=100, le=30_000)
    agents: dict[str, SixiangAgentConfig] = Field(
        default_factory=dict,
        description="Per-agent model overrides (mingjue, weaver, yang, yin, anqu)",
    )
...
class AgentDefaults(Base):
    ...
    sixiang: SixiangConfig = Field(default_factory=SixiangConfig)
...
class TpnToolConfig(Base):
    """TPN (Task Positive Network) — sixiang control & goal management tool."""

    enable: bool = True  # register the `tpn` tool


class DmnToolConfig(Base):
    """DMN (Default Mode Network) — cognitive evolution manager tool."""

    enable: bool = True  # register the `dmn` tool
...
class ToolsConfig(Base):
    ...
    tpn: TpnToolConfig = Field(default_factory=TpnToolConfig)
    dmn: DmnToolConfig = Field(default_factory=DmnToolConfig)
...
class Config(BaseSettings):
    """Root configuration for vingobot."""
    ...
    def workspace_path(self) -> Path:
        ...
        Delegates to :func:`vingobot.config.paths.get_workspace_path`
        so relative paths (e.g. ``".vingobot"``) are resolved against
        the project root rather than CWD.
        """
    ...
    def _match_provider(
        ...
        from vingobot.providers.registry import PROVIDERS, find_by_name
    ...
    model_config = ConfigDict(env_prefix="VINGOBOT_", env_nested_delimiter="__")
