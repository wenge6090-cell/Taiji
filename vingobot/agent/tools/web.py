...
from vingobot.utils.helpers import build_image_content_blocks

if TYPE_CHECKING:
    from vingobot.config.schema import WebFetchConfig, WebSearchConfig
...
class WebFetchTool(Tool):
    ...
    def __init__(self, config: WebFetchConfig | None = None, proxy: str | None = None, user_agent: str | None = None, max_chars: int = 50000):
        from vingobot.config.schema import WebFetchConfig
        ...
        try:
            async with httpx.AsyncClient(proxy=self.proxy, follow_redirects=True, max_redirects=MAX_REDIRECTS, timeout=15.0) as client:
                async with client.stream("GET", url, headers={"User-Agent": self.user_agent}) as r:
                    from vingobot.security.network import validate_resolved_url
