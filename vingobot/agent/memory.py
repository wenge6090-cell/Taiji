...
from vingobot.utils.prompt_templates import render_template
from vingobot.utils.helpers import ensure_dir, estimate_message_tokens, estimate_prompt_tokens_chain, strip_think, truncate_text

from vingobot.agent.runner import AgentRunSpec, AgentRunner
from vingobot.agent.tools.registry import ToolRegistry
from vingobot.utils.gitstore import GitStore
...
class Consolidator:
    ...
    def __init__(
        ...
        *,
        ...
        # Share the AgentLoop's session locks so Consolidator and _dispatch()
        # block on the same lock, preventing concurrent session.save() calls.
    ...
    def get_lock(self, session_key: str) -> asyncio.Lock:
        """Return the shared lock for one session.

        When ``session_locks`` was provided (shared from ``AgentLoop``),
        use the same lock that ``_dispatch()`` holds, preventing concurrent
        ``session.save()`` from Consolidator and the main dispatch path.
        Otherwise, fall back to the internal ``WeakValueDictionary``.
        """
    ...
    async def _consolidate_loop(
        self,
        session: Session,
        *,
        session_summary: str | None = None,
    ) -> None:
        """Core consolidation loop: archive old messages until prompt fits within safe budget."""
        budget = self._input_token_budget
    ...
    async def maybe_consolidate_by_tokens(
        ...
        *,
        ...
        When *_already_locked* is True, the caller already holds the
        session lock (e.g. ``_dispatch`` via ``_process_message``) and we
        skip the internal ``async with lock`` to avoid an ``asyncio.Lock``
        deadlock.
        """
        ...
        if _already_locked:
            await self._consolidate_loop(session, session_summary=session_summary)
        else:
...
class Dream:
    ...
    def _build_tools(self) -> ToolRegistry:
        ...
        from vingobot.agent.tools.file_state import FileStates
        from vingobot.agent.tools.filesystem import EditFileTool, ReadFileTool, WriteFileTool

        tools = ToolRegistry()
        workspace = self.store.workspace
        skills_dir = workspace / ".taiji" / "cognition" / "skills"
        # Allow reading skills for reference during skill creation
        ...
        # write under .taiji/cognition/skills/ so the prompt can safely use
        # .taiji/cognition/skills/<name>/SKILL.md.
    ...
    def _list_existing_skills(self) -> list[str]:
        ...
        skills_dir = _resolve_builtin_skills_dir()
    ...
    async def run(self) -> bool:
        ...
        tools = self._tools
        skill_creator_path = _resolve_builtin_skills_dir() / "skill-creator" / "SKILL.md"
