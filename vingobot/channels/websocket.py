"""WebSocket server channel: vingobot acts as a WebSocket server and serves connected clients."""
...
from vingobot.utils.helpers import safe_filename
from vingobot.utils.media_decode import (
    FileSizeExceeded,
...
class WebSocketConfig(Base):
    ...
    - ``token_issue_path``: If non-empty, **GET** (HTTP/1.1) to this path returns JSON
      ...
      vingobot and shares the asyncio loop, use a thread or async HTTP client for GET—do not call
      blocking ``urllib`` or synchronous ``httpx`` from inside a coroutine.
    - ``token_issue_secret``: If non-empty, token requests must send ``Authorization: Bearer <secret>`` or
      ``X-vingobot-Auth: <secret>``.
...
def _issue_route_secret_matches(headers: Any, configured_secret: str) -> bool:
    ...
    header_token = headers.get("X-vingobot-Auth") or headers.get("x-vingobot-auth")
    if not header_token:
        return False
...
class WebSocketChannel(BaseChannel):
    ...
    def _handle_webui_bootstrap(self, connection: Any) -> Response:
        if self.config.websocket_requires_token and not _is_localhost(connection):
    ...
    def _settings_payload(self, *, requires_restart: bool = False) -> dict[str, Any]:
        from vingobot.config.loader import get_config_path, load_config
