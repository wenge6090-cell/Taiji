...
from vingobot.config.paths import get_media_dir, get_runtime_subdir
from vingobot.config.schema import Base
from vingobot.utils.helpers import split_message
...
class WeixinConfig(Base):
    ...
    state_dir: str = ""  # Default: <data-dir>/weixin/ (from get_runtime_subdir)
...
class WeixinChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        if self.config.token:
            ...
            if not await self._qr_login():
                logger.error("WeChat login failed. Run 'vingobot channels login weixin' to authenticate.")
                self._running = False
                return
    ...
    async def _send_text(
        ...
        client_id = f"vingobot-{uuid.uuid4().hex[:12]}"
