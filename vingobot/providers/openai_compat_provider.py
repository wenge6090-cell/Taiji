...
from vingobot.providers.base import LLMProvider, LLMResponse, ToolCallRequest
from vingobot.providers.openai_responses import (
    consume_sdk_stream,
    convert_messages,
    convert_tools,
...
if TYPE_CHECKING:
    from vingobot.providers.registry import ProviderSpec
...
_DEFAULT_OPENROUTER_HEADERS = {
    "HTTP-Referer": "https://vingobot.local",
    "X-OpenRouter-Title": "vingobot",
...
def _uses_openrouter_attribution(spec: "ProviderSpec | None", api_base: str | None) -> bool:
    """Apply vingobot attribution headers to OpenRouter requests by default."""
...
class OpenAICompatProvider(LLMProvider):
    ...
    def _build_kwargs(
        ...
        # (e.g. vLLM) receive them, without breaking the OpenAI client's
        # strict parameter validation.
        _extra: dict[str, Any] = {}
        if top_k is not None:
            _extra["top_k"] = top_k
        if repetition_penalty is not None:
            _extra["repetition_penalty"] = repetition_penalty
        if _extra:
            existing = kwargs.get("extra_body", {})
            kwargs["extra_body"] = _deep_merge(existing, _extra)
    ...
    async def chat_stream(
        ...
        idle_timeout_s = int(os.environ.get("VINGOBOT_STREAM_IDLE_TIMEOUT_S", "90"))
        try:
            if self._should_use_responses_api(model, reasoning_effort):
                try:
                    body = self._build_responses_body(
