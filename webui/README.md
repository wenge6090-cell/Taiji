...
The browser front-end for the vingobot gateway. It is built with Vite + React 18 +
...
> checkout.
>
> WebUI changes may land before they are included in the next packaged release,
> so source installs and published package versions are not yet guaranteed
> to move in lockstep.
...
vingobot/web/dist/      build output served by the gateway
```

## Develop from source

### 1. Install vingobot from source
...
```bash
VINGOBOT_API_URL=http://127.0.0.1:9000 bun run dev
```
...
This writes the production assets to `../vingobot/web/dist`, which is the
directory served by `vingobot gateway` and bundled into the Python wheel.
