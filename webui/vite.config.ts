...
export default defineConfig(({ mode }) => {
  ...
  const target = env.VINGOBOT_API_URL ?? "http://127.0.0.1:8765";
  ...
  return {
    ...
    build: {
      outDir: path.resolve(__dirname, "../vingobot/web/dist"),
      emptyOutDir: true,
      sourcemap: false,
    },
    server: {
      ...
      // the ``/`` proxy below (Vite HMR and the vingobot ws upgrade both sit on
      ...
      proxy: {
        ...
        // Forward only WebSocket upgrades on ``/`` to the vingobot gateway;
