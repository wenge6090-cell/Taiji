...
import { usevingobotStream } from "@/hooks/usevingobotStream";
...
function wrap(client: ReturnType<typeof fakeClient>["client"]) {
  return function Wrapper({ children }: { children: ReactNode }) {
    return (
      <ClientProvider
        client={client as unknown as import("@/lib/vingobot-client").vingobotClient}
        token="tok"
      >
        {children}
      </ClientProvider>
...
describe("usevingobotStream", () => {
  it("collapses consecutive tool_hint frames into one trace row", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => usevingobotStream("chat-t", []), {
  ...
  it("attaches assistant media_urls to complete messages", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => usevingobotStream("chat-m", []), {
      wrapper: wrap(fake.client),
    });
  ...
  it("keeps assistant buttons on complete messages", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => usevingobotStream("chat-q", []), {
      wrapper: wrap(fake.client),
    });
