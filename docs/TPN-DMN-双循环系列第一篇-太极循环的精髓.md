...
明觉(Mingjue) → 编织器(Weaver) → 阳(Yang/执行意图)
  → 阴(Yin/审批) → 执行器(Executor) → 暗驱(Anqu/反思+路由决策)
```
...
| 爻位 | 角色 | 职责 | 类比人类 |
|------|------|------|---------|
| 初爻 · 明觉 | 感知者+思变决策 | 评估目标进展，查看蓝图和完成标准；选择卦象、细化任务 | "我现在在哪？下一步怎么办？" |
...
| 上爻 · 暗驱 | 分析师+路由决策 | 分析执行结果，提取经验模式，做出路由决策 | "我学到了什么？目标达成了吗？" |
...
第1轮·暗驱：分析 → "获取了5条规模数据，但缺少政策维度"；路由决策：goal_next_task
```

DMN 并行分析日志，提炼模式：**"做市场分析时，应先收集行业规模、政策、技术三个维度的数据，再综合撰写。"** 这个模式被写入 L2 模型库。

```
第2轮·明觉：评估进展 + 思变决策 → "已收集规模数据，需要政策数据"；卦象：巽卦
第2轮·Weaver：感知到 L2 有新的市场分析模式 → 调整策略
第2轮·执行：搜索"光伏2025政策" + 写入临时数据文件分类整理
# 第2轮·暗驱：分析 → "政策数据已获取，技术数据待补充"；路由决策：goal_next_task
```
