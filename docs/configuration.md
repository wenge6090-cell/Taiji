...
Config file: `~/.vingobot/config.json`

> [!NOTE]
> If your config file is older than the current schema, you can refresh it without overwriting your existing values:
> run `vingobot onboard`, then answer `N` when asked whether to overwrite the config.
> vingobot will merge in missing default fields and keep your current settings.
...
```ini
# /etc/systemd/system/vingobot.service (excerpt)
[Service]
EnvironmentFile=/home/youruser/VINGOBOT_secrets.env
User=vingobot
ExecStart=...
```

```bash
# /home/youruser/VINGOBOT_secrets.env (mode 600, owned by youruser)
...
| `volcengine` | LLM (VolcEngine, pay-per-use) | [volcengine.com](https://www.volcengine.com) |
| `byteplus` | LLM (VolcEngine international, pay-per-use) | [byteplus.com](https://www.byteplus.com) |
...
| `openai_codex` | LLM (Codex, OAuth) | `vingobot provider login openai-codex` |
| `github_copilot` | LLM (GitHub Copilot, OAuth) | `vingobot provider login github-copilot` |
...
You can also set `providers.bedrock.apiKey` to a Bedrock API key; vingobot exports it as `AWS_BEARER_TOKEN_BEDROCK` for the AWS SDK.
...
Claude Opus 4.7 does not accept `temperature`, `top_p`, or `top_k`; vingobot omits `temperature` automatically for this model. If `reasoningEffort` is set to `low`, `medium`, `high`, `max`, or `adaptive`, vingobot sends Bedrock's adaptive thinking parameter.
...
Use Bedrock model IDs or inference profile IDs with a `bedrock/` prefix in vingobot config. vingobot removes the prefix before calling AWS.
...
Model-specific fields can be supplied with `extraBody`; vingobot merges it into Converse `additionalModelRequestFields`:
...
Current scope: vingobot passes `messages`, `system`, `inferenceConfig`, `toolConfig`, and `additionalModelRequestFields`. Bedrock Prompt Management, Guardrails, `serviceTier`, and other top-level Converse options are not first-class config fields yet.
...
```bash
vingobot agent -m "Reply with one short sentence."
```
...
No `providers.openaiCodex` block is needed in `config.json`; `vingobot provider login` stores the OAuth session outside config.

**1. Login:**
```bash
vingobot provider login openai-codex
```

**2. Set model** (merge into `~/.vingobot/config.json`):
```json
{
  "agents": {
    "defaults": {
...
**3. Chat:**
```bash
vingobot agent -m "Hello!"

# Target a specific workspace/config locally
vingobot agent -c ~/.vingobot-telegram/config.json -m "Hello!"

# One-off workspace override on top of that config
vingobot agent -c ~/.vingobot-telegram/config.json -w /tmp/vingobot-telegram-test -m "Hello!"
```
...
No `providers.githubCopilot` block is needed in `config.json`; `vingobot provider login` stores the OAuth session outside config.

**1. Login:**
```bash
vingobot provider login github-copilot
```
...
LongCat is available through vingobot's built-in OpenAI-compatible provider flow.
...
Some OpenAI-compatible gateways expose request-body extensions such as vLLM guided decoding or local sampling controls. Put those under `extraBody`; vingobot merges them into the chat-completions request body after its provider defaults:
...
**2. Add to config** (partial — merge into `~/.vingobot/config.json`):
```json
{
  "providers": {
    "ollama": {
...
**3. Add to config** (partial — merge into `~/.vingobot/config.json`):
...
vingobot uses a **Provider Registry** (`vingobot/providers/registry.py`) as the single source of truth.
Adding a new provider only takes **2 steps** — no if-elif chains to touch.

**Step 1.** Add a `ProviderSpec` entry to `PROVIDERS` in `vingobot/providers/registry.py`:
...
ProviderSpec(
    ...
    display_name="My Provider",          # shown in `vingobot status`
    default_api_base="https://api.myprovider.com/v1",  # OpenAI-compatible endpoint
)
```

**Step 2.** Add a field to `ProvidersConfig` in `vingobot/config/schema.py`:
...
That's it! Environment variables, model routing, config matching, and `vingobot status` display will all work automatically.
...
Global settings that apply to all channels. Configure under the `channels` section in `~/.vingobot/config.json`:
...
When a channel `send()` raises, vingobot retries at the channel-manager layer. By default, `channels.sendMaxRetries` is `3`, and that count includes the initial send.
...
>
> If a channel is completely unreachable, vingobot cannot notify the user through that same channel. Watch logs for `Failed to send to {channel} after N attempts` to spot persistent delivery failures.

## Web Tools

vingobot incorporates basic tools for accessing the web. These include searching via APIs, and fetching arbitrary web pages in Markdown format. They are enabled by default, and can be configured in `~/.vingobot/config.json` under `tools.web`.
...
vingobot supports multiple web search providers. Configure in `~/.vingobot/config.json` under `tools.web.search`.
...
vingobot by default uses [Jina Reader](https://jina.ai/reader/), a third-party API, to convert arbitrary pages into Markdown format for easy digestion by the LLM, with a local fallback based on [readability-lxml](https://github.com/buriy/python-readability) if the former fails.
...
vingobot supports [MCP](https://modelcontextprotocol.io/) — connect external tool servers and use them as native agent tools.
...
`enabledTools` accepts either the raw MCP tool name (for example `read_file`) or the wrapped vingobot tool name (for example `mcp_filesystem_write_file`).
...
**Docker security**: The official Docker image runs as a non-root user (`vingobot`, UID 1000) with bubblewrap pre-installed. When using `docker-compose.yml`, the container drops all Linux capabilities except `SYS_ADMIN` (required for bwrap's namespace isolation).


## Auto Compact

When a user is idle for longer than a configured threshold, vingobot **proactively** compresses the older part of the session context into a summary while keeping a recent legal suffix of live messages. This reduces token cost and first-token latency when the user returns — instead of re-processing a long stale context with an expired KV cache, the model receives a compact summary, the most recent live context, and fresh input.
...
By default, vingobot uses `UTC` for runtime time context. If you want the agent to think in your local time, set `agents.defaults.timezone` to a valid [IANA timezone name](https://en.wikipedia.org/wiki/List_of_tz_database_time_zones):
...
By default, each channel × chat ID combination gets its own session. If you use vingobot across multiple channels (e.g. Telegram + Discord + CLI) and want them to share the same conversation, enable `unifiedSession`:
...
vingobot ships with built-in skills, and your workspace can also define custom skills under `skills/`. If you want to hide specific skills from the agent, set `agents.defaults.disabledSkills` to a list of skill directory names:
