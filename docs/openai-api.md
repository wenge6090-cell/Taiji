...
vingobot can expose a minimal OpenAI-compatible endpoint for local integrations:

```bash
pip install "vingobot[api]"
vingobot serve
```
...
If `channel` points to a channel that is not enabled in your config, vingobot will queue the outbound event but no platform delivery will occur.
