...
Use vingobot as a library — no CLI, no gateway, just Python.
...
`vingobot.from_config()` reuses your normal `~/.vingobot/config.json`, so the SDK follows the same provider, model, tools, and workspace defaults as the CLI unless you override them.
...
bot = vingobot.from_config(
    config_path="~/.vingobot/config.json",
...
Hooks let you inspect tool calls, streaming, and iteration state without modifying vingobot internals:

```python
from vingobot.agent import AgentHook, AgentHookContext
...
### `vingobot.from_config(config_path=None, *, workspace=None)`

Create a `vingobot` instance from a config file.

| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `config_path` | `str \| Path \| None` | `None` | Path to `config.json`. Defaults to `~/.vingobot/config.json`. |
...
```python
from vingobot.agent import AgentHook
...
async def main() -> None:
    bot = vingobot.from_config(workspace="/my/project")
