...
graph TD
    subgraph "太极: 跨三国旅行规划的多轮认知螺旋"
        ...
        T4 --> T5["第5轮: 思变·决策<br/>分析: 用户信息不足是因为问题太宽泛<br/>决策: 下一轮用更具体的提问策略<br/>姿态切换: 离卦·明照"]
        ...
        T8 --> T9["第9轮: 思变·决策<br/>决策: 开始生成行程草案<br/>姿态切换: 乾卦·创造"]
        ...
        T11 --> T12["第12轮: 思变·决策<br/>分析: 需要插入缓冲时间<br/>决策: 用坎卦深潜分析交通<br/>姿态切换: 坎卦·险陷"]
        
        T12 --> T13["第13轮: Weaver→行动<br/>姿态: 坎卦·险陷<br/>工具: deep_analyze<br/>产出: 优化后的交通方案"]
        
        T13 --> T14["第14轮: 思变·决策<br/>决策: 重新生成行程<br/>姿态切换: 乾卦·创造"]
        T14 --> T15["第15轮: 行动<br/>产出: 优化版行程"]
        
        T15 --> T16["第16轮: 阴→暗驱→思变<br/>DMN扫描: 学到了'跨三国旅行=需要先充分收集偏好'<br/>更新L2模型: 旅行规划任务的初始提问策略<br/>判定: ✅ 目标完成"]
    end
...
3. **自主决策**——思变每轮决定"下一步做什么"，不需要预定义图结构
...
graph LR
    ...
    subgraph "太极的循环"
        direction TB
        T1["第N轮: 认知状态S₁<br/>姿态: PN"] -->|执行+审批| T2[思变分析]
...
- 动作空间 A：八卦认知姿态（8种策略）
- 转移函数 T：六爻循环（明觉→织→阳→阴→行动→暗驱→思变）
- 策略 π：思变决策（基于当前状态选择下一个姿态）
