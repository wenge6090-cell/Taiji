...
Connect vingobot to your favorite chat platform. Want to build your own? See the [Channel Plugin Guide](./channel-plugin-guide.md).

| Channel | What you need |
|---------|---------------|
| **Telegram** | Bot token from @BotFather |
| **Discord** | Bot token + Message Content intent |
| **WhatsApp** | QR code scan (`vingobot channels login whatsapp`) |
| **WeChat (Weixin)** | QR code scan (`vingobot channels login weixin`) |
...
**1. Ask vingobot to set up Mochat for you**

Simply send this message to vingobot (replace `xxx@xxx` with your real email):

```
Register on MoChat (https://mochat.dev). My Email account is xxx@xxx Bind me as your owner and DM me on MoChat.
```

vingobot will automatically register, configure `~/.vingobot/config.json`, and connect to Mochat.
...
That's it — vingobot handles the rest!
...
If you prefer to configure manually, add the following to `~/.vingobot/config.json`:
...
- You need:
  - `userId` (example: `@vingobot:matrix.org`)
  - `password`
...
> After upgrading vingobot, rebuild the local bridge with:
> `rm -rf ~/.vingobot/bridge && vingobot channels login whatsapp`
...
- **Permissions**:
  ...
  - **Streaming replies** (default in vingobot): add **`cardkit:card:write`** (often labeled **Create and update cards** in the Feishu developer console). Required for CardKit entities and streamed assistant text. Older apps may not have it yet — open **Permission management**, enable the scope, then **publish** a new app version if the console requires it.
  - If you **cannot** add `cardkit:card:write`, set `"streaming": false` under `channels.feishu` (see below). The bot still works; replies use normal interactive cards without token-by-token streaming.
- **Events**: Add `im.message.receive_v1` (receive messages)
  - Select **Long Connection** mode (requires running vingobot first to establish connection)
...
> `allowFrom`: Add your open_id (find it in vingobot logs when you message the bot). Use `["*"]` to allow all users.
...
> - `allowFrom`: Add your openid (find it in vingobot logs when you message the bot). Use `["*"]` for public access.
...
> `files:read` is required to read files users send to vingobot. `files:write` is required for vingobot to send images, videos, and other file uploads. If you add either scope later, reinstall the Slack app to the workspace and restart vingobot so it uses the updated bot token.

**3. Configure vingobot**
...
Give vingobot its own email account. It polls **IMAP** for incoming mail and replies via **SMTP** — like a personal email assistant.

**1. Get credentials (Gmail example)**
- Create a dedicated Gmail account for your bot (e.g. `my-vingobot@gmail.com`)
...
```json
{
  "channels": {
    "email": {
      ...
      "imapPort": 993,
      "imapUsername": "my-vingobot@gmail.com",
      "imapPassword": "your-app-password",
      "smtpHost": "smtp.gmail.com",
      "smtpPort": 587,
      "smtpUsername": "my-vingobot@gmail.com",
      "smtpPassword": "your-app-password",
      "fromAddress": "my-vingobot@gmail.com",
...
> - `allowFrom`: Add the sender ID you see in vingobot logs for your WeChat account. Use `["*"]` to allow all users.
> - `token`: Optional. If omitted, log in interactively and vingobot will save the token for you.
> - `routeTag`: Optional. When your upstream Weixin deployment requires request routing, vingobot will send it as the `SKRouteTag` header.
> - `stateDir`: Optional. Defaults to vingobot's runtime directory for Weixin state.
> - `pollTimeout`: Optional long-poll timeout in seconds.

**3. Login**

```bash
vingobot channels login weixin
```

Use `--force` to re-authenticate and ignore any saved token:

```bash
vingobot channels login weixin --force
```
...
> - `mentionOnlyResponse` controls what vingobot receives when a user sends only a bot mention (`<at>vingobot</at>`). Set to `""` to ignore mention-only messages.
