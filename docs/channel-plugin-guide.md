...
Build a custom vingobot channel in three steps: subclass, package, install.

> **Note:** We recommend developing channel plugins against a source checkout of vingobot (`pip install -e .`) rather than a PyPI release, so you always have access to the latest base-channel features and APIs.

## How It Works

vingobot discovers channel plugins via Python [entry points](https://packaging.python.org/en/latest/specifications/entry-points/). When `vingobot gateway` starts, it scans:

1. Built-in channels in `vingobot/channels/`
2. External packages registered under the `vingobot.channels` entry point group
...
```text
vingobot-channel-webhook/
├── VINGOBOT_channel_webhook/
...
```python
# VINGOBOT_channel_webhook/__init__.py
from VINGOBOT_channel_webhook.channel import WebhookChannel

__all__ = ["WebhookChannel"]
```

```python
# VINGOBOT_channel_webhook/channel.py
import asyncio
...
# pyproject.toml
[project]
name = "vingobot-channel-webhook"
version = "0.1.0"
dependencies = ["vingobot", "aiohttp"]

[project.entry-points."vingobot.channels"]
webhook = "VINGOBOT_channel_webhook:WebhookChannel"
...
packages = ["VINGOBOT_channel_webhook"]
```
...
```bash
pip install -e .
vingobot plugins list      # verify "Webhook" shows as "plugin"
vingobot onboard           # auto-adds default config for detected plugins
```

Edit `~/.vingobot/config.json`:
...
```bash
vingobot channels login <channel_name>
vingobot channels login <channel_name> --force  # re-authenticate
```
...
| `default_config()` (classmethod) | Returns default config dict for `vingobot onboard`. Override to declare your fields. |
...
Built-in channels use Pydantic config models (subclassing `Base` from `vingobot.config.schema`). Plugin channels **must do the same**.

### Pattern

1. Define a Pydantic model inheriting from `vingobot.config.schema.Base`:
...
Override `default_config()` so `vingobot onboard` auto-populates `config.json`:
...
| What | Format | Example |
|------|--------|---------|
| PyPI package | `vingobot-channel-{name}` | `vingobot-channel-webhook` |
| Entry point key | `{name}` | `webhook` |
| Config section | `channels.{name}` | `channels.webhook` |
| Python package | `VINGOBOT_channel_{name}` | `VINGOBOT_channel_webhook` |

## Local Development

```bash
git clone https://github.com/you/vingobot-channel-webhook
cd vingobot-channel-webhook
pip install -e .
vingobot plugins list    # should show "Webhook" as "plugin"
vingobot gateway         # test end-to-end
```

## Verify

```bash
$ vingobot plugins list
