...
echo "vingobot line count"
echo "=================="
echo ""

echo "Core runtime"
echo "------------"
core_agent=$(count_top_level_py_lines "vingobot/agent")
core_bus=$(count_top_level_py_lines "vingobot/bus")
core_config=$(count_top_level_py_lines "vingobot/config")
core_cron=$(count_top_level_py_lines "vingobot/cron")
core_heartbeat=$(count_top_level_py_lines "vingobot/heartbeat")
core_session=$(count_top_level_py_lines "vingobot/session")
...
echo "----------------"
extra_tools=$(count_recursive_py_lines "vingobot/agent/tools")
extra_skills=$(count_skill_lines "vingobot/skills")
extra_api=$(count_recursive_py_lines "vingobot/api")
extra_cli=$(count_recursive_py_lines "vingobot/cli")
extra_channels=$(count_recursive_py_lines "vingobot/channels")
extra_utils=$(count_recursive_py_lines "vingobot/utils")
...
echo "Notes"
echo "-----"
echo "  - agent/ only counts top-level Python files under vingobot/agent"
echo "  - tools/ is counted separately from vingobot/agent/tools"
echo "  - skills/ counts .md, .py, and .sh files"
echo "  - not included here: command/, providers/, security/, templates/, vingobot.py, root files"
