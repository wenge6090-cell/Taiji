...
from vingobot.channels.telegram import (
...
class _FakeBot:
    ...
    async def get_me(self):
        self.get_me_calls += 1
        return SimpleNamespace(id=999, username="VINGOBOT_test")
...
async def test_start_creates_separate_pools_with_proxy(monkeypatch) -> None:
    ...
    monkeypatch.setattr("vingobot.channels.telegram.HTTPXRequest", _FakeHTTPXRequest)
    monkeypatch.setattr(
        "vingobot.channels.telegram.Application",
...
async def test_send_text_retries_on_timeout() -> None:
    ...
    import vingobot.channels.telegram as tg_mod
...
async def test_on_error_logs_network_issues_as_warning(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "vingobot.channels.telegram.logger.warning",
        lambda message, error: recorded.append(("warning", message.format(error))),
    )
    monkeypatch.setattr(
        "vingobot.channels.telegram.logger.error",
...
async def test_send_delta_stream_end_html_expansion_does_not_overflow() -> None:
    ...
    """
    from vingobot.channels.telegram import _markdown_to_telegram_html
...
async def test_send_delta_incremental_edit_splits_oversized_buffer() -> None:
    ...
    from vingobot.channels.telegram import TELEGRAM_MAX_MESSAGE_LEN
...
async def test_send_remote_media_url_after_security_validation(monkeypatch) -> None:
    ...
    channel._app = _FakeApp(lambda: None)
    monkeypatch.setattr("vingobot.channels.telegram.validate_url_target", lambda url: (True, ""))
...
async def test_send_blocks_unsafe_remote_media_url(monkeypatch) -> None:
    ...
    channel._app = _FakeApp(lambda: None)
    monkeypatch.setattr(
        "vingobot.channels.telegram.validate_url_target",
...
async def test_group_policy_mention_accepts_text_mention_and_caches_bot_identity() -> None:
    ...
    await channel._on_message(_make_telegram_update(text="@VINGOBOT_test hi", entities=[mention]), None)
    await channel._on_message(_make_telegram_update(text="@VINGOBOT_test again", entities=[mention]), None)
...
async def test_group_policy_mention_accepts_caption_mention() -> None:
    ...
    await channel._on_message(
        _make_telegram_update(caption="@VINGOBOT_test photo", caption_entities=[mention]),
        None,
    )

    assert len(handled) == 1
    assert handled[0]["content"] == "@VINGOBOT_test photo"
...
async def test_download_message_media_returns_path_when_download_succeeds(
    ...
    monkeypatch.setattr(
        "vingobot.channels.telegram.get_media_dir",
...
async def test_forward_command_preserves_dream_log_args_and_strips_bot_suffix() -> None:
    ...
    channel._handle_message = capture_handle
    update = _make_telegram_update(text="/dream-log@VINGOBOT_test deadbeef", reply_to_message=None)
...
async def test_forward_command_normalizes_telegram_safe_dream_aliases() -> None:
    ...
    channel._handle_message = capture_handle
    update = _make_telegram_update(text="/dream_restore@VINGOBOT_test deadbeef", reply_to_message=None)
...
def test_strip_md_block_removes_inline_formatting() -> None:
    from vingobot.channels.telegram import _strip_md_block
