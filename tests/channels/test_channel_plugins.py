...
from vingobot.config.schema import ChannelsConfig
from vingobot.providers.transcription import GroqTranscriptionProvider as _GroqProvider
from vingobot.providers.transcription import OpenAITranscriptionProvider as _OpenAIProvider
from vingobot.utils.restart import RestartNotice
...
def test_discover_plugins_loads_entry_points():
    from vingobot.channels.registry import discover_plugins
...
async def test_base_channel_passes_api_base_to_openai_transcription_provider():
    ...
    from vingobot.providers import transcription as transcription_mod
...
def test_openai_transcription_provider_honors_api_base_argument():
    from vingobot.providers.transcription import OpenAITranscriptionProvider
...
async def test_transcription_provider_includes_language(tmp_path, provider_cls, language):
    ...
    with patch("vingobot.providers.transcription.httpx.AsyncClient", return_value=_stub_async_client(captured)):
...
def test_channels_login_uses_discovered_plugin_class(monkeypatch):
    ...
    from vingobot.cli.commands import app
    ...
    monkeypatch.setattr("vingobot.config.loader.load_config", lambda config_path=None: Config())
    monkeypatch.setattr(
        "vingobot.channels.registry.discover_all",
...
def test_builtin_channel_default_config():
    ...
    from vingobot.channels.telegram import TelegramChannel
...
async def test_send_with_retry_retries_on_failure():
    ...
    with patch("vingobot.channels.manager.asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
        await mgr._send_with_retry(mgr.channels["failing"], msg)
...
async def test_send_with_retry_no_retry_when_max_is_zero():
    ...
    with patch("vingobot.channels.manager.asyncio.sleep", new_callable=AsyncMock):
        await mgr._send_with_retry(mgr.channels["failing"], msg)
...
async def test_send_with_retry_propagates_cancelled_error_during_sleep():
    ...
    with patch("vingobot.channels.manager.asyncio.sleep", side_effect=cancel_during_sleep):
        with pytest.raises(asyncio.CancelledError):
            await mgr._send_with_retry(mgr.channels["failing"], msg)
...
async def test_notify_restart_done_enqueues_outbound_message():
    ...
    with patch("vingobot.channels.manager.consume_restart_notice_from_env", return_value=notice):
