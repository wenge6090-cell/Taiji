...
from vingobot.channels.websocket import (
    WebSocketChannel,
    WebSocketConfig,
    _is_valid_chat_id,
...
def test_issue_route_secret_matches_bearer_and_header() -> None:
    ...
    x_headers = Headers([("X-vingobot-Auth", "my-secret")])
...
async def test_send_stages_external_media_as_signed_url(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr("vingobot.channels.websocket.get_media_dir", fake_media_dir)
    channel = WebSocketChannel({"enabled": True, "allowFrom": ["*"]}, bus)
    mock_ws = AsyncMock()
    channel._attach(mock_ws, "chat-1")
...
def test_registry_discovers_websocket_channel() -> None:
    from vingobot.channels.registry import load_channel_class
...
async def test_settings_api_returns_safe_subset_and_updates_whitelist(
    ...
    monkeypatch.setattr("vingobot.config.loader._current_config_path", config_path)
