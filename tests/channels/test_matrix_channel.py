...
from vingobot.channels.matrix import _build_matrix_text_content

import vingobot.channels.matrix as matrix_module
from vingobot.bus.events import OutboundMessage
from vingobot.bus.queue import MessageBus
from vingobot.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from vingobot.channels.matrix import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("vingobot.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "vingobot.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("vingobot.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "vingobot.channels.matrix.asyncio.create_task", _fake_create_task
    )
