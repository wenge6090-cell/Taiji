...
try:
    from vingobot.channels import dingtalk
...
import vingobot.channels.dingtalk as dingtalk_module
from vingobot.bus.queue import MessageBus
from vingobot.channels.dingtalk import DingTalkChannel, DingTalkConfig, vingobotDingTalkHandler
...
async def test_group_send_uses_group_messages_api() -> None:
    ...
    ok = await channel._send_batch_message(
        ...
        "group:conv123",
        "sampleMarkdown",
        {"text": "hello", "title": "vingobot Reply"},
    )
...
async def test_handler_uses_voice_recognition_text_when_text_is_empty(monkeypatch) -> None:
    ...
    handler = vingobotDingTalkHandler(channel)
...
async def test_handler_processes_file_message(monkeypatch) -> None:
    ...
    async def fake_download(download_code, filename, sender_id):
        return f"/tmp/VINGOBOT_dingtalk/{sender_id}/{filename}"
    ...
    assert "/tmp/VINGOBOT_dingtalk/user1/report.xlsx" in msg.content
...
async def test_download_dingtalk_file(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "vingobot.config.paths.get_media_dir",
