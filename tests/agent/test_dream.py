...
from vingobot.agent.runner import AgentRunResult
from vingobot.agent.skills import _resolve_builtin_skills_dir
from vingobot.utils.gitstore import LineAge
...
class TestDreamRun:
    ...
    async def test_skill_phase_uses_builtin_skill_creator_path(self, dream, mock_provider, mock_runner, store):
        ...
        expected = str(_resolve_builtin_skills_dir() / "skill-creator" / "SKILL.md")
        assert expected in system_prompt

    async def test_skill_write_tool_accepts_workspace_relative_skill_path(self, dream, store):
        """Dream skill creation should allow .taiji/cognition/skills/<name>/SKILL.md."""
        write_tool = dream._tools.get("write_file")
        assert write_tool is not None

        result = await write_tool.execute(
            path=".taiji/cognition/skills/test-skill/SKILL.md",
            content="---\nname: test-skill\ndescription: Test\n---\n",
        )

        assert "Successfully wrote" in result
        assert (store.workspace / ".taiji" / "cognition" / "skills" / "test-skill" / "SKILL.md").exists()
...
class TestDreamPromptCaps:
    ...
    async def test_phase1_caps_huge_history_entry(
        ...
        # record that was written by an older vingobot build before any caps.
