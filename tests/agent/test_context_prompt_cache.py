...
def test_bootstrap_files_are_backed_by_templates() -> None:
    template_dir = pkg_files("vingobot") / "templates"
...
def test_identity_has_no_behavioral_instructions(tmp_path) -> None:
    ...
    assert "You are vingobot" not in identity
...
def test_default_soul_template_contains_execution_rules() -> None:
    ...
    soul = (pkg_files("vingobot") / "templates" / "SOUL.md").read_text(encoding="utf-8")
...
def test_always_skills_excluded_from_skills_index(tmp_path, monkeypatch) -> None:
    """Always skills should appear in Active Skills but NOT in the skills index."""
    workspace = _make_workspace(tmp_path)

    # Patch skills dir to point to the test workspace
    skills_dir = workspace / ".taiji" / "cognition" / "skills"
    monkeypatch.setattr("vingobot.agent.skills._resolve_builtin_skills_dir", lambda: skills_dir)

    # Create a memory skill with always=true in the .taiji skills directory
    memory_skill_dir = skills_dir / "memory"
    memory_skill_dir.mkdir(parents=True)
    (memory_skill_dir / "SKILL.md").write_text(
        "---\nalways: true\ndescription: Memory management helper\n---\n\n"
        "# Memory\n\nGuidance for recalling and organising information.",
        encoding="utf-8",
    )
...
def test_template_memory_md_is_skipped(tmp_path) -> None:
    ...
    assert "This file is automatically updated by vingobot" not in prompt
