...
from vingobot.providers.base import LLMResponse, ToolCallRequest
...
def _make_loop(tmp_path):
    ...
    with patch("vingobot.agent.loop.ContextBuilder"), \
         patch("vingobot.agent.loop.SessionManager"), \
         patch("vingobot.agent.loop.SubagentManager") as MockSubMgr:
...
async def test_runner_preserves_reasoning_fields_and_tool_results():
    from vingobot.agent.runner import AgentRunSpec, AgentRunner
...
async def test_runner_calls_hooks_in_order():
    from vingobot.agent.hook import AgentHook, AgentHookContext
    from vingobot.agent.runner import AgentRunSpec, AgentRunner
...
def test_persist_tool_result_prunes_old_session_buckets(tmp_path):
    from vingobot.utils.helpers import maybe_persist_tool_result
...
def test_persist_tool_result_logs_cleanup_failures(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "vingobot.utils.helpers._cleanup_tool_result_buckets",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(OSError("busy")),
    )
    monkeypatch.setattr(
        "vingobot.utils.helpers.logger.warning",
...
def test_snip_history_drops_orphaned_tool_results_from_trimmed_slice(monkeypatch):
    ...
    monkeypatch.setattr("vingobot.agent.runner.estimate_prompt_tokens_chain", lambda *_args, **_kwargs: (500, None))
    token_sizes = {
        "old user": 120,
        "tool call": 120,
        "tool output": 40,
    ...
    monkeypatch.setattr(
        "vingobot.agent.runner.estimate_message_tokens",
...
async def test_runner_keeps_going_when_tool_result_persistence_fails():
    ...
    runner = AgentRunner(provider)
    with patch("vingobot.agent.runner.maybe_persist_tool_result", side_effect=RuntimeError("disk full")):
        result = await runner.run(AgentRunSpec(
            initial_messages=[{"role": "user", "content": "do task"}],
            tools=tools,
            model="test-model",
...
async def test_llm_error_not_appended_to_session_messages():
    ...
    from vingobot.agent.runner import (
        AgentRunSpec,
        AgentRunner,
...
async def test_next_turn_after_llm_error_keeps_turn_boundary(tmp_path):
    from vingobot.agent.loop import AgentLoop
    from vingobot.agent.runner import _PERSISTED_MODEL_ERROR_PLACEHOLDER
    from vingobot.bus.events import InboundMessage
    from vingobot.bus.queue import MessageBus
...
async def test_subagent_max_iterations_announces_existing_fallback(tmp_path, monkeypatch):
    from vingobot.agent.subagent import SubagentManager, SubagentStatus
    from vingobot.bus.queue import MessageBus
    ...
    monkeypatch.setattr("vingobot.agent.tools.filesystem.ListDirTool.execute", fake_execute)
...
async def test_length_recovery_gives_up_after_max_retries():
    ...
    from vingobot.agent.runner import AgentRunSpec, AgentRunner, _MAX_LENGTH_RECOVERIES
...
async def test_backfill_missing_tool_results_inserts_error():
    ...
    from vingobot.agent.runner import AgentRunner, _BACKFILL_CONTENT
...
def test_drop_orphan_tool_results_removes_unmatched_tool_messages():
    from vingobot.agent.runner import AgentRunner
...
async def test_backfill_repairs_model_context_without_shifting_save_turn_boundary(tmp_path):
    ...
    from vingobot.agent.loop import AgentLoop
    from vingobot.agent.runner import _BACKFILL_CONTENT
    from vingobot.bus.events import InboundMessage
    from vingobot.bus.queue import MessageBus
...
async def test_runner_backfill_only_mutates_model_context_not_returned_messages():
    ...
    from vingobot.agent.runner import AgentRunSpec, AgentRunner, _BACKFILL_CONTENT
...
async def test_microcompact_replaces_old_tool_results():
    ...
    from vingobot.agent.runner import AgentRunner, _MICROCOMPACT_KEEP_RECENT
...
async def test_drain_injections_passes_limit_to_callback_when_supported():
    ...
    from vingobot.agent.runner import AgentRunSpec, AgentRunner, _MAX_INJECTIONS_PER_TURN
    from vingobot.bus.events import InboundMessage
...
async def test_injection_cycles_capped_at_max():
    ...
    from vingobot.agent.runner import AgentRunSpec, AgentRunner, _MAX_INJECTION_CYCLES
    from vingobot.bus.events import InboundMessage
...
async def test_pending_queue_preserves_overflow_for_next_injection_cycle(tmp_path):
    ...
    from vingobot.bus.events import InboundMessage
    from vingobot.bus.queue import MessageBus
    from vingobot.agent.runner import _MAX_INJECTIONS_PER_TURN
...
async def test_drain_injections_on_empty_final_response():
    ...
    from vingobot.agent.runner import AgentRunSpec, AgentRunner, _MAX_EMPTY_RETRIES
    from vingobot.bus.events import InboundMessage
...
def test_snip_history_preserves_user_message_after_truncation(monkeypatch):
    ...
    messages = [
        ...
        {"role": "user", "content": ".vingobot的同目录"},
        {
            "role": "assistant",
            "content": None,
    ...
    monkeypatch.setattr("vingobot.agent.runner.estimate_prompt_tokens_chain", lambda *_a, **_kw: (500, None))
