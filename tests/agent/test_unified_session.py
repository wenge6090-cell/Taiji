...
from vingobot.command.builtin import cmd_new, register_builtin_commands
from vingobot.command.router import CommandContext, CommandRouter
from vingobot.config.schema import AgentDefaults, Config
...
def _make_loop(tmp_path: Path, unified_session: bool = False) -> AgentLoop:
    ...
    with patch("vingobot.agent.loop.SessionManager"), \
         patch("vingobot.agent.loop.SubagentManager") as MockSubMgr, \
         patch("vingobot.agent.loop.Dream"):
...
class TestUnifiedSessionConfig:
    ...
    def test_save_config_writes_unified_session(self, tmp_path: Path):
        """save_config() writes 'unifiedSession' into config.json."""
        ...
        agents_defaults = data["agents"]["defaults"]
        assert "unifiedSession" in agents_defaults, (
            "saved config.json must contain 'unifiedSession' key"
        )
        assert agents_defaults["unifiedSession"] is False
...
class TestConsolidationUnaffectedByUnifiedSession:
    ...
    async def test_consolidation_skips_empty_session_for_unified_key(self):
        ...
        from vingobot.agent.memory import Consolidator, MemoryStore
