...
class TestBuildEnvUnix:
    ...
    def test_expected_keys(self):
        with patch("vingobot.agent.tools.shell._IS_WINDOWS", False):
    ...
    def test_secrets_excluded(self, monkeypatch):
        ...
        monkeypatch.setenv("VINGOBOT_TOKEN", "tok-secret")
        with patch("vingobot.agent.tools.shell._IS_WINDOWS", False):
            env = ExecTool()._build_env()
        assert "OPENAI_API_KEY" not in env
        assert "VINGOBOT_TOKEN" not in env
        for v in env.values():
...
class TestBuildEnvWindows:
    ...
    def test_expected_keys(self):
        with patch("vingobot.agent.tools.shell._IS_WINDOWS", True):
            env = ExecTool()._build_env()
    ...
    def test_path_has_sensible_default(self):
        with (
            patch("vingobot.agent.tools.shell._IS_WINDOWS", True),
...
class TestSpawnUnix:
    ...
    async def test_uses_bash(self):
        with (
            patch("vingobot.agent.tools.shell._IS_WINDOWS", False),
...
class TestPathAppendPlatform:
    ...
    async def test_unix_uses_env_var_in_fixed_export(self):
        ...
        with (
            patch("vingobot.agent.tools.shell._IS_WINDOWS", False),
            patch("vingobot.agent.tools.shell.os.pathsep", ":"),
        ...
        assert captured_cmd == 'export PATH="$PATH:$VINGOBOT_PATH_APPEND"; ls'
        assert captured_env["VINGOBOT_PATH_APPEND"] == "/opt/bin; echo INJECTED"
...
class TestSandboxPlatform:
    ...
    async def test_bwrap_applied_on_unix(self):
        ...
        with (
            patch("vingobot.agent.tools.shell._IS_WINDOWS", False),
            patch("vingobot.agent.tools.shell.wrap_command", return_value="bwrap -- sh -c ls") as mock_wrap,
            patch.object(ExecTool, "_spawn", return_value=mock_proc) as mock_spawn,
            patch.object(ExecTool, "_guard_command", return_value=None),
        ):
