...
from vingobot.agent.tools.web import WebFetchTool
...
async def test_web_fetch_blocks_private_ip():
    tool = WebFetchTool()
    with patch("vingobot.security.network.socket.getaddrinfo", _fake_resolve_private):
...
async def test_web_fetch_blocks_localhost():
    ...
    with patch("vingobot.security.network.socket.getaddrinfo", _resolve_localhost):
...
async def test_web_fetch_result_contains_untrusted_flag():
    ...
    with patch("vingobot.security.network.socket.getaddrinfo", _fake_resolve_public), \
...
async def test_web_fetch_can_skip_jina_and_use_custom_user_agent(monkeypatch):
    tool = WebFetchTool(
        ...
        user_agent="vingobot-test-agent",
    )
    ...
    monkeypatch.setattr("vingobot.agent.tools.web.httpx.AsyncClient", FakeClient)
