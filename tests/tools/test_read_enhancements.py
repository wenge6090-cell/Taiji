...
from vingobot.agent.tools.filesystem import ReadFileTool, WriteFileTool
from vingobot.agent.tools import file_state
...
class TestReadOfficeDocuments:
    ...
    async def test_docx_returns_extracted_text(self, tool, tmp_path):
        with patch("vingobot.utils.document.extract_text", return_value="Title\n\nParagraph 1"):
    ...
    async def test_xlsx_returns_extracted_text(self, tool, tmp_path):
        with patch("vingobot.utils.document.extract_text", return_value="--- Sheet: Sheet1 ---\nName\tAge\nAlice\t30"):
    ...
    async def test_pptx_returns_extracted_text(self, tool, tmp_path):
        with patch("vingobot.utils.document.extract_text", return_value="--- Slide 1 ---\nWelcome\n--- Slide 2 ---\nContent"):
    ...
    async def test_docx_missing_library(self, tool, tmp_path):
        with patch("vingobot.utils.document.extract_text", return_value="[error: python-docx not installed]"):
            f = tmp_path / "test.docx"
            f.write_bytes(b"PK")
            result = await tool.execute(path=str(f))
        assert "Error" in result
    ...
    async def test_docx_corrupt_file(self, tool, tmp_path):
        with patch("vingobot.utils.document.extract_text", return_value="[error: failed to extract DOCX: bad zip]"):
            f = tmp_path / "test.docx"
    ...
    async def test_unsupported_extension(self, tool, tmp_path):
        with patch("vingobot.utils.document.extract_text", return_value=None):
            f = tmp_path / "test.docx"
            f.write_bytes(b"PK")
            result = await tool.execute(path=str(f))
        assert "Error" in result
    ...
    async def test_empty_document_returns_descriptive_message(self, tool, tmp_path):
        with patch("vingobot.utils.document.extract_text", return_value=""):
...
class TestOfficeDocTruncation:
    ...
    async def test_large_document_truncated(self, tool, tmp_path):
        with patch("vingobot.utils.document.extract_text", return_value="x" * 200_000):
    ...
    async def test_small_document_not_truncated(self, tool, tmp_path):
        with patch("vingobot.utils.document.extract_text", return_value="Hello world"):
    ...
    async def test_error_response_not_truncated(self, tool, tmp_path):
        with patch("vingobot.utils.document.extract_text", return_value="[error: failed to extract DOCX: something went wrong]"):
            f = tmp_path / "bad.docx"
            f.write_bytes(b"PK")
            result = await tool.execute(path=str(f))
        assert "Error" in result
