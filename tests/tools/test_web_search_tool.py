...
from vingobot.agent.tools.web import WebSearchTool
from vingobot.config.schema import WebSearchConfig
...
async def test_brave_search(monkeypatch):
    async def mock_get(self, url, **kw):
        ...
        assert kw["headers"]["User-Agent"] == "vingobot-search-test"
        return _response(json={
            "web": {"results": [{"title": "vingobot", "url": "https://example.com", "description": "AI assistant"}]}
        })

    monkeypatch.setattr(httpx.AsyncClient, "get", mock_get)
    tool = _tool(provider="brave", api_key="brave-key", user_agent="vingobot-search-test")
    result = await tool.execute(query="vingobot", count=1)
    assert "vingobot" in result
...
async def test_tavily_search(monkeypatch):
    ...
    tool = _tool(provider="tavily", api_key="tavily-key", user_agent="vingobot-search-test")
...
async def test_searxng_search(monkeypatch):
    ...
    monkeypatch.setattr(httpx.AsyncClient, "get", mock_get)
    tool = _tool(provider="searxng", base_url="https://searx.example", user_agent="vingobot-search-test")
...
async def test_duckduckgo_search(monkeypatch):
    ...
    monkeypatch.setattr("vingobot.agent.tools.web.DDGS", MockDDGS, raising=False)
    import vingobot.agent.tools.web as web_mod
...
async def test_jina_search(monkeypatch):
    ...
    monkeypatch.setattr(httpx.AsyncClient, "get", mock_get)
    tool = _tool(provider="jina", api_key="jina-key", user_agent="vingobot-search-test")
    result = await tool.execute(query="test")
...
async def test_kagi_search(monkeypatch):
    ...
    monkeypatch.setattr(httpx.AsyncClient, "get", mock_get)
    tool = _tool(provider="kagi", api_key="kagi-key", user_agent="vingobot-search-test")
