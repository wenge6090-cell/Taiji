...
from vingobot.cli.commands import _make_provider, app
...
from vingobot.providers.openai_codex_provider import _strip_model_prefix
...
def test_first_run_creates_config_non_interactive(tmp_path, monkeypatch):
    """_first_run_setup should create a config file without API key in non-TTY mode."""
    from vingobot.cli.commands import _first_run_setup

    config_path = tmp_path / "config.json"
    monkeypatch.setattr("sys.stdin.isatty", lambda: False)
    monkeypatch.setattr("vingobot.channels.registry.discover_all", lambda: {})

    _first_run_setup(config_path)

    assert config_path.exists()
    saved = json.loads(config_path.read_text(encoding="utf-8"))
    assert "agents" in saved


def test_init_creates_config_non_interactive(tmp_path, monkeypatch):
    """init command should create a config file in non-TTY mode."""
    config_path = tmp_path / "init-config.json"
    monkeypatch.setattr("sys.stdin.isatty", lambda: False)
    monkeypatch.setattr("vingobot.channels.registry.discover_all", lambda: {})

    result = runner.invoke(app, ["init", "--config", str(config_path), "--yes"])
...
def test_init_with_workspace_sets_custom_workspace(tmp_path, monkeypatch):
    """init --workspace should set custom workspace in config."""
    config_path = tmp_path / "ws-config.json"
    workspace_dir = tmp_path / "my-workspace"
    monkeypatch.setattr("sys.stdin.isatty", lambda: False)
    monkeypatch.setattr("vingobot.channels.registry.discover_all", lambda: {})
    monkeypatch.setattr("vingobot.core.workspace.init_workspace", lambda **kw: MagicMock(root=tmp_path / ".taiji"))

    result = runner.invoke(
        app,
        ["init", "--config", str(config_path), "--workspace", str(workspace_dir), "--yes"],
    )
...
def test_init_initializes_taiji_structure(tmp_path, monkeypatch):
    """init command should create .taiji cognition structure in the workspace."""
    config_path = tmp_path / "taiji-config.json"
    workspace_dir = tmp_path / "init-workspace"
    monkeypatch.setattr("sys.stdin.isatty", lambda: False)
    monkeypatch.setattr("vingobot.channels.registry.discover_all", lambda: {})
    ...
    assert result.exit_code == 0
    taiji = workspace_dir.resolve() / ".taiji"
    assert (taiji / "cognition" / "skills").is_dir()
    assert (taiji / "cognition" / "models").is_dir()
    assert (taiji / "cognition" / "grids").is_dir()
    assert (taiji / "cognition" / "truths").is_dir()
    assert (taiji / "goals").is_dir()
...
def test_openai_compat_provider_passes_model_through():
    ...
    with patch("vingobot.providers.openai_compat_provider.AsyncOpenAI"):
...
def test_make_provider_uses_github_copilot_backend():
    from vingobot.cli.commands import _make_provider
    from vingobot.config.schema import Config
...
async def test_github_copilot_provider_refreshes_client_api_key_before_chat():
    ...
    with patch("vingobot.providers.openai_compat_provider.AsyncOpenAI", return_value=mock_client):
...
def test_make_provider_passes_extra_headers_to_custom_provider():
    ...
    with patch("vingobot.providers.openai_compat_provider.AsyncOpenAI") as mock_async_openai:
...
def mock_agent_runtime(tmp_path):
    ...
    with patch("vingobot.config.loader.load_config", return_value=config) as mock_load_config, \
         patch("vingobot.config.loader.resolve_config_env_vars", side_effect=lambda c: c), \
         patch("vingobot.cli.commands.sync_workspace_templates") as mock_sync_templates, \
         patch("vingobot.cli.commands._make_provider", return_value=object()), \
         patch("vingobot.cli.commands._print_agent_response") as mock_print_response, \
         patch("vingobot.bus.queue.MessageBus"), \
         patch("vingobot.cron.service.CronService"), \
         patch("vingobot.agent.loop.AgentLoop") as mock_agent_loop_cls:
...
def test_agent_uses_default_config_when_no_workspace_or_config_flags(mock_agent_runtime):
    ...
    assert result.exit_code == 0
    # load_config is now always called with a path (auto-created if missing)
    assert mock_agent_runtime["load_config"].called
...
def test_agent_config_sets_active_path(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "vingobot.config.loader.set_config_path",
        lambda path: seen.__setitem__("config_path", path),
    )
    monkeypatch.setattr("vingobot.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("vingobot.cli.commands.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("vingobot.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("vingobot.bus.queue.MessageBus", lambda: object())
    monkeypatch.setattr("vingobot.cron.service.CronService", lambda _store: object())
    ...
    monkeypatch.setattr("vingobot.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("vingobot.cli.commands._print_agent_response", lambda *_args, **_kwargs: None)
...
def test_agent_uses_workspace_directory_for_cron_store(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("vingobot.config.loader.set_config_path", lambda _path: None)
    monkeypatch.setattr("vingobot.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("vingobot.cli.commands.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("vingobot.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("vingobot.bus.queue.MessageBus", lambda: object())
    ...
    monkeypatch.setattr("vingobot.cron.service.CronService", _FakeCron)
    monkeypatch.setattr("vingobot.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("vingobot.cli.commands._print_agent_response", lambda *_args, **_kwargs: None)
...
def test_agent_workspace_override_does_not_migrate_legacy_cron(
    ...
    monkeypatch.setattr("vingobot.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("vingobot.bus.queue.MessageBus", lambda: object())
    monkeypatch.setattr("vingobot.config.paths.get_cron_dir", lambda: legacy_dir)
...
def test_agent_custom_config_workspace_does_not_migrate_legacy_cron(
    ...
    monkeypatch.setattr("vingobot.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr(
        "vingobot.cli.commands._print_agent_response", lambda *_args, **_kwargs: None
    )
...
def _patch_cli_command_runtime(
    ...
    *,
    ...
    monkeypatch.setattr("vingobot.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("vingobot.config.loader.resolve_config_env_vars", lambda c: c)
    monkeypatch.setattr(
        "vingobot.cli.commands.sync_workspace_templates",
        sync_templates or (lambda _path: None),
    )
    monkeypatch.setattr(
        "vingobot.cli.commands._make_provider",
        provider_factory,
    )
    monkeypatch.setattr(
        "vingobot.providers.factory.build_provider_snapshot",
        lambda _config: _test_provider_snapshot(provider_factory(_config), _config),
    )
    monkeypatch.setattr(
        "vingobot.providers.factory.load_provider_snapshot",
        lambda _config_path=None: _test_provider_snapshot(provider_factory(config), config),
    )

    if message_bus is not None:
        monkeypatch.setattr("vingobot.bus.queue.MessageBus", message_bus)
    if session_manager is not None:
        monkeypatch.setattr("vingobot.session.manager.SessionManager", session_manager)
    if cron_service is not None:
        monkeypatch.setattr("vingobot.cron.service.CronService", cron_service)
    if get_cron_dir is not None:
        monkeypatch.setattr("vingobot.config.paths.get_cron_dir", get_cron_dir)
...
def _patch_serve_runtime(monkeypatch, config: Config, seen: dict[str, object]) -> None:
    ...
    monkeypatch.setattr("vingobot.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("vingobot.api.server.create_app", _fake_create_app)
...
def test_gateway_cron_evaluator_receives_scheduled_reminder_context(
    ...
    monkeypatch.setattr("vingobot.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("vingobot.cli.commands.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("vingobot.cli.commands._make_provider", lambda _config: provider)
    monkeypatch.setattr(
        "vingobot.providers.factory.build_provider_snapshot",
    ...
    monkeypatch.setattr("vingobot.bus.queue.MessageBus", lambda: bus)
    ...
    monkeypatch.setattr("vingobot.session.manager.SessionManager", _FakeSessionManager)
    ...
    monkeypatch.setattr("vingobot.cron.service.CronService", _FakeCron)
    monkeypatch.setattr("vingobot.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("vingobot.channels.manager.ChannelManager", _StopAfterCronSetup)
    monkeypatch.setattr(
        "vingobot.utils.evaluator.evaluate_response",
...
def test_gateway_cron_job_suppresses_intermediate_progress(
    ...
    monkeypatch.setattr("vingobot.bus.queue.MessageBus", lambda: bus)
    monkeypatch.setattr("vingobot.session.manager.SessionManager", lambda _workspace: object())
...
def test_migrate_cron_store_moves_legacy_file(tmp_path: Path) -> None:
    ...
    from vingobot.cli.commands import _migrate_cron_store
    ...
    with patch("vingobot.config.paths.get_cron_dir", return_value=legacy_dir):
...
def test_gateway_health_endpoint_binds_and_serves_expected_responses(
    ...
    monkeypatch.setattr("vingobot.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("vingobot.channels.manager.ChannelManager", _FakeChannelManager)
    monkeypatch.setattr("vingobot.cron.service.CronService", _FakeCronService)
    monkeypatch.setattr("vingobot.heartbeat.service.HeartbeatService", _FakeHeartbeatService)
