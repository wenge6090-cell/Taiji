"""Tests for the vingobot programmatic facade."""
...
from vingobot.vingobot import vingobot, RunResult
...
def test_from_config_missing_file():
    with pytest.raises(FileNotFoundError):
        vingobot.from_config("/nonexistent/config.json")


def test_from_config_creates_instance(tmp_path):
    config_path = _write_config(tmp_path)
    bot = vingobot.from_config(config_path, workspace=tmp_path)
...
def test_from_config_default_path():
    ...
    with patch("vingobot.config.loader.load_config") as mock_load, \
         patch("vingobot.vingobot._make_provider") as mock_prov:
        mock_load.return_value = Config()
        mock_prov.return_value = MagicMock()
        mock_prov.return_value.get_default_model.return_value = "test"
        mock_prov.return_value.generation.max_tokens = 4096
        vingobot.from_config()
...
def test_workspace_override(tmp_path):
    ...
    bot = vingobot.from_config(config_path, workspace=custom_ws)
    assert bot._loop.workspace == custom_ws


def test_sdk_make_provider_uses_github_copilot_backend():
    from vingobot.config.schema import Config
    from vingobot.vingobot import _make_provider
...
def test_import_from_top_level():
    from vingobot import vingobot as N, RunResult as R
    assert N is vingobot
