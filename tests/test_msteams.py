...
try:
    from vingobot.channels import msteams
    MSTEAMS_AVAILABLE = getattr(msteams, "MSTEAMS_AVAILABLE", False)
except ImportError:
    MSTEAMS_AVAILABLE = False

if not MSTEAMS_AVAILABLE:
    pytest.skip("MSTeams dependencies not installed (PyJWT, cryptography). Run: pip install vingobot[msteams]", allow_module_level=True)

import jwt
from cryptography.hazmat.primitives.asymmetric import rsa

import vingobot.channels.msteams as msteams_module
from vingobot.bus.events import OutboundMessage
from vingobot.channels.msteams import ConversationRef, MSTeamsChannel
...
def make_channel(tmp_path, monkeypatch):
    monkeypatch.setattr("vingobot.channels.msteams.get_workspace_path", lambda: tmp_path)
...
def test_strip_possible_bot_mention_removes_generic_at_tags(make_channel):
    ...
    assert ch._strip_possible_bot_mention("<at>vingobot</at> hello") == "hello"
...
def test_sanitize_inbound_text_keeps_normal_inline_message(make_channel):
    ...
    activity = {
        "text": "<at>vingobot</at> normal inline message",
        "channelData": {},
    }
...
async def test_start_logs_install_hint_when_pyjwt_missing(make_channel, monkeypatch):
    ...
    assert errors == ["PyJWT not installed. Run: pip install vingobot[msteams]"]
